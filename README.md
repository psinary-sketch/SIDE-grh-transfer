...
Lean 4 kernel for the **complete seven-voice chorus + SIDE structural exhaustiveness for L(s, χ)** of GRH transfer — all seven mechanism-class voices of the SIDE method, transferred from ξ(s) to L(s, χ) via Mathlib's `DirichletCharacter` API, and assembled through Ostrowski's classification into the composite SIDE structural exhaustiveness theorem mirroring `SIDE-kernel/Bridge/TheBridgeComplete.lean`.
...
### GRHBridge (composite SIDE structural exhaustiveness for L(s, χ)) — `GRHBridge.lean`

**New in v0.5.0.** The seven-voice paired chorus assembled with Ostrowski's classification into the SIDE structural exhaustiveness statement for L(s, χ). The bridge defines the seven mechanism classes (`MechanismClass` enum: `C1_schwarz` through `C7_hadamard`), the per-class `produces_offline_chi` algebraic signature each mechanism would imprint at an off-line zero, the seven per-class exclusion theorems (`c1_chi_exclusion` through `c7_chi_exclusion`) each invoking the corresponding v0.4.0 paired voice theorem, the `none_produce_chi` combiner, the χ-independent `ostrowski_exhaustive_chi` (since the places of ℚ don't depend on χ), and the three-conjunct `GRHStructuralExhaustiveness` proposition discharged by **`grh_structural_exhaustiveness_proved`**.
Mirrors `structural_exhaustiveness_proved` in `SIDE-kernel/Bridge/TheBridgeComplete.lean` on the ξ side — the SIDE-method's voice in Lean for L(s, χ). The kernel now witnesses, end-to-end, the seven-voice + Ostrowski SIDE structural exhaustiveness statement for any pair of Dirichlet characters (χ, χbar) mod n.

## Scope

This is the **complete seven-voice chorus + composite SIDE structural exhaustiveness theorem** for the GRH transfer — six iff-σ=1/2 voices (Voice1 + Voice2-paired + Voice3-paired + Voice3b-paired + Voice5-paired + Voice6-paired) plus one σ-inert voice (Voice7-paired) for chorus exhaustiveness, all wired through Ostrowski's classification of absolute values on ℚ into the composite `grh_structural_exhaustiveness_proved` theorem.

The SIDE structural exhaustiveness statement for L(s, χ) is now kernel-verified: 7 mechanism classes, none produces an off-line zero, Ostrowski exhausts the places. The "exhaustiveness/exclusion architecture transfer" named as open in v0.4.0 is **closed in v0.5.0**.
**Remaining open work** (v0.6.0): the Conservation-based Route 3 analog — paired-form Conservation of Spectra for L(s, χ) delivering `(h_cons : ConservationHypothesisChi) → RiemannHypothesisChi` in the Wiles+Hales pattern (mirroring `SIDE-kernel/Bridge/ConservationBridge.lean` on the ξ side). Separate downstream target, not blocking the structural exhaustiveness theorem just delivered.

**Spec note.** Voice3b/Voice5/Voice6 (v0.3.0), Voice7 (v0.4.0), and the GRHBridge composite assembly (v0.5.0) are **not enumerated in `LEAN_VERIFICATIONS_TODO_v0_1.md`** — only LV-H-6 (Voice3 + Voice2 paired) was specifically flagged by the audit-arc. The chorus-completion + composite-assembly framing is the author's broader research direction beyond the specced compile-list; these were transferred because the algebraic core was tractable and the design pattern (vendor + typed-paired delegation + SIDE syllogism assembly) had been established by LV-H-6 and the SIDE-kernel ξ-side bridge.
## Build
...
**GRHBridge (`GRHBridge.lean`):**
- `Rat.AbsoluteValue.equiv_real_or_padic` (`Mathlib.NumberTheory.Ostrowski`) — Ostrowski's classification of nontrivial absolute values on ℚ as either real or p-adic. Used in `ostrowski_exhaustive_chi` to discharge the third conjunct of `GRHStructuralExhaustiveness`. Character-independent: the places of ℚ don't depend on χ, so this re-uses the same Mathlib lemma the ξ-side bridge does.
...
# expected output: each of 10 theorems depends on axioms:
...
`GRHBridge.lean` is **NOT vendored** — it imports the paired voice theorems from this kernel's own `Character*.lean` files and uses `Mathlib.NumberTheory.Ostrowski` directly. It mirrors the structure of `SIDE-kernel/Bridge/TheBridgeComplete.lean` but is an original assembly for L(s, χ), not a copy.
...
The CharacterSymmetry / CharacterCodim / CharacterModular / CharacterSpectral / CharacterTopological substance files use `Complex.sub_re` / `Complex.one_re` / `Complex.ofReal_re` from Mathlib's complex arithmetic where needed; the (χ, χbar)-paired form signatures use `DirichletCharacter ℂ n` from the same Mathlib infrastructure. `GRHBridge.lean` additionally uses `Mathlib.NumberTheory.Ostrowski` for the χ-independent places-of-ℚ classification.
## Cross-references
...
- **Compile-list targets:** `LEAN_VERIFICATIONS_TODO_v0_1.md#LV-M-6` (closed v0.1.0); `LEAN_VERIFICATIONS_TODO_v0_1.md#LV-H-6` (closed v0.2.0). **Voice3b/Voice5/Voice6 (v0.3.0), Voice7 (v0.4.0), and GRHBridge (v0.5.0) are not specified** in the TODO; treated as author-directed broader-plan work.
- **Downstream open targets:** Conservation-based Route 3 analog for L(s, χ) — paired-form Conservation of Spectra delivering `(h_cons : ConservationHypothesisChi) → RiemannHypothesisChi` (the Wiles+Hales pattern, mirroring `SIDE-kernel/Bridge/ConservationBridge.lean` on the ξ side); `LV-L-5` (Landau-Siegel, trivial-after-GRH).
...
- **v0.3.0** (2026-05-28): Adds Voice3b / Voice5 / Voice6 paired forms via `CharacterCodim.lean`, `CharacterModular.lean`, `CharacterSpectral.lean`, with vendoring through `Voice3bVendored.lean`, `Voice5Vendored.lean`, `Voice6Vendored.lean`. Adds `paired_cr_minimal_codim_axis_iff`, `paired_modular_S_fixed_iff`, `paired_spectral_offset_zero_iff`. **Transitively closes Voice2 codim** (Voice3b algebra = deferred Voice2 codim algebra). Total: 8 theorems, 0 sorry, 0 custom axioms. Chorus at 6-of-7 voices. Voice3b/Voice5/Voice6 transfers are author-directed broader-plan work beyond the LV-* spec-tracked compile-list. Tag `v0.3.0` at commit `cdf201a`.
- **v0.4.0** (2026-05-28): Adds Voice7 (topological inertness, paired form) via `CharacterTopological.lean`, with vendoring through `Voice7Vendored.lean`. Adds `paired_topological_no_sigma_preference`. **Structurally inverted** from the other six voices: not an iff-σ=1/2 theorem but a constant-equality (topological_contribution σ = topological_contribution (1/2), both equal because the contribution is constant). Voice7 proves σ-INERTNESS — the topological class cannot place zeros at any specific σ. Completes the **7-of-7 voice chorus**. Total: 9 theorems, 0 sorry, 0 custom axioms. Tag `v0.4.0` at commit `68ec127a`.
- **v0.5.0** (2026-05-29): Adds `GRHBridge.lean` — the SIDE-method composite assembly for L(s, χ). Defines a 7-constructor `MechanismClass` enum (mirroring `SIDE-kernel/Bridge/TheBridgeComplete.lean`), the per-class `produces_offline_chi` algebraic signature, seven `cN_chi_exclusion` theorems each invoking the corresponding v0.4.0 paired voice theorem, the `none_produce_chi` combiner, the χ-independent `ostrowski_exhaustive_chi` (using `Rat.AbsoluteValue.equiv_real_or_padic` from `Mathlib.NumberTheory.Ostrowski`), the three-conjunct `GRHStructuralExhaustiveness` proposition, and the headline **`grh_structural_exhaustiveness_proved`** theorem discharging it unconditionally. Total: 10 theorems, 0 sorry, 0 custom axioms. The SIDE structural exhaustiveness statement for L(s, χ) is now kernel-verified — the "exhaustiveness/exclusion architecture transfer" named as open in v0.4.0 is closed in v0.5.0. Remaining open: Conservation-based Route 3 analog for the Wiles+Hales-pattern final assembly (v0.6.0).
